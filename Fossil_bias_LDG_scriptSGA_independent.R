...
Mam_25rs <- mam_df_land[,c("X", "Y",Combi_25rs$iucn2020_binomial, "Land_mass", 
                           "Clim_zone","GUM_USC", "WC_zone", "Richness", "Land", 
                           "Area", "Fossils")] #1435
Mam_50rs <- mam_df_land[,c("X", "Y",Combi_50rs$iucn2020_binomial, "Land_mass", 
                           "Clim_zone", "GUM_USC", "WC_zone", "Richness", "Land", 
                           "Area","Fossils")] #2860
Mam_75rs <- mam_df_land[,c("X", "Y",Combi_75rs$iucn2020_binomial, "Land_mass", 
                           "Clim_zone", "GUM_USC", "WC_zone", "Richness", "Land", 
                           "Area","Fossils")] #4284
...
richness <- apply(Mam_25rs[,3:1427], 1, function(x) length(x[x>0])) 
...
} #Richness values stored in "Filters_alone_richness.csv"
...
Mam_75bs <- mam_df_land[,c("X", "Y",Combi_75bs$iucn2020_binomial, "Land_mass", 
                           "Clim_zone", "GUM_USC", "WC_zone", "Richness", "Land", 
                           "Area", "Fossils")] #4134
...
Mam_50bs <- mam_df_land[,c("X", "Y",Combi_50bs$iucn2020_binomial, "Land_mass", 
                           "Clim_zone", "GUM_USC", "WC_zone", "Richness", "Land", 
                           "Area", "Fossils")] #2760
...
Mam_25bs <- mam_df_land[,c("X", "Y",Combi_25bs$iucn2020_binomial, "Land_mass", 
                           "Clim_zone", "GUM_USC", "WC_zone", "Richness", "Land", 
                           "Area", "Fossils")] #1385
...
length(which(colSums(Mam_fos[,3:5703]) == 0)) #1267 spp lost (+2) 5701-1267 = 4434 spp
...
#6 families in Combi_75bs are not in Mam_pbdb 
sort(table(Combi_2[Combi_2$family %in% spp_Combir, "family"])) #8 spp
spp_Mampbdb <- sort(setdiff(unique(Mam_pbdb$family), unique(Combi_2$family)))
#474 families in Mam_pbdb are not in Combi_r

Combi_2 <- Combi_2[!Combi_2$family %in% spp_Combir,] #5691 obs, 133 families, 8 spp lost
Mam_pbdb3 <- Mam_pbdb[!Mam_pbdb$family %in% spp_Mampbdb,] # 133 families
...
Mam_75tax <- mam_df_land[,c("X", "Y",Combi_75tax$iucn2020_binomial, "Land_mass", 
                            "Clim_zone", "GUM_USC", "WC_zone", "Richness", "Land", 
                            "Area", "Fossils")] #4278
...
round(0.75*nrow(Mam_pbdb3)) #100 familiEs
...
Mam_75tax2 <- mam_df_land[,c("X", "Y",Combi_3$iucn2020_binomial, "Land_mass", 
                             "Clim_zone", "GUM_USC", "WC_zone", "Richness", "Land", 
                             "Area", "Fossils")] 
...
## Taxonomic "whole-families"
round(0.5*nrow(Mam_pbdb3)) #66 families
sort(Mam_pbdb3$n_obs, decreasing = T)[[66]]
my_families <- Mam_pbdb3[Mam_pbdb3$n_obs >=82,1]
Combi_3 <- Combi_2[Combi_2$family %in% my_families,]

Mam_50tax2 <- mam_df_land[,c("X", "Y",Combi_3$iucn2020_binomial, "Land_mass", 
                             "Clim_zone", "GUM_USC", "WC_zone", "Richness", "Land", 
                             "Area", "Fossils")] 
...
Combi_3 <- Combi_2[Combi_2$family %in% my_families,]
Mam_25tax2 <- mam_df_land[,c("X", "Y",Combi_3$iucn2020_binomial, "Land_mass", 
                             "Clim_zone", "GUM_USC", "WC_zone", "Richness", "Land", 
                             "Area", "Fossils")] 
...
db_forslopes <- data.frame(mam_r_dfforplot$Longlat, mam_r_dfforplot$Moll,
                           ...
                           Mam50bs_moll_sf$last, Mam25bs_moll_sf$last, Mamfos_moll_sf$last, 
                           Mam_75tax_moll_sf$last, Mam_50tax_moll_sf$last,
                           Mam_25tax_moll_sf$last, Mam_75tax2_moll_sf$last, 
...
#pdf("LDG_Mam_25bs_indep.pdf", useDingbats = F, width = 10, height = 7)
ggplot(db_forslopes, aes(Longlat, bs25, color = bs25)) +
  geom_point(shape = 16,
             size = 3, alpha = 0.9) +
...
terra::plot(Mam_25tax2_moll, col = rev(paletteer_c("grDevices::YlOrBr", 400)), 
